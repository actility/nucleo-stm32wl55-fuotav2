            ...
            case FW_MANAGEMENT_DEV_REBOOT_TIME_REQ:
                {
                    ...
                    else
                    {
                        ...
                        /* rebootTimeReq is GPS time */
                        curTime = SysTimeGet();

                        rebootTimeAns = rebootTimeReq - (curTime.Seconds - UNIX_GPS_EPOCH_OFFSET);
                        MW_LOG(TS_OFF, VLEVEL_M, "Current time epoch: %u, reboot time GPS: %u, ans time: %d\r\n",
                        		curTime.Seconds, rebootTimeReq, rebootTimeAns);
