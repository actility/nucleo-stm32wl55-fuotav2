...
uint32_t FLASH_IF_PAGE_Index (uint32_t pStart)
{
   ...
   else if (IS_FLASH_EXT_MEM_ADDRESS(pStart))
   {
     return (pStart - FLASH_IF_EXT_FLASH_ADDRESS) / MX25L4006_SECTOR_SIZE;
   }
   return 0;
}
...
static FLASH_IF_StatusTypedef FLASH_IF_EXT_Write(void *pDestination, const void *pSource, uint32_t uLength)
{
  ...
  for (sector_index = start_sector_index; sector_index < (start_sector_index + number_sectors); sector_index++)
  {
    ...
    if (FLASH_IF_EXT_IsEmpty((void *)uDest, length) != FLASH_IF_MEM_EMPTY)
    {
      if (pAllocatedBuffer == NULL)
      {
        ret_status = FLASH_IF_PARAM_ERROR;
