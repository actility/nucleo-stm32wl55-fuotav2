...
int32_t FRAG_DECODER_IF_Write(uint32_t addr, uint8_t *data, uint32_t size)
{
  ...
  {
    APP_LOG(TS_OFF, VLEVEL_M, "\r\n.... !! FLASH_IF_WRITE_ERROR: %d at %x for %d !! ....\r\n", status,
    		FRAG_DECODER_DWL_REGION_START + addr, size);
  }
#endif /* INTEROP_TEST_MODE */
...
static bool verify_magic (uint32_t datafile, uint32_t magic)
{
	uint32_t tmpmagic = 0;

	if( FLASH_IF_Read(&tmpmagic, (void *)datafile, sizeof(uint32_t)) != FLASH_IF_OK )
	{
		APP_LOG(TS_OFF, VLEVEL_M, "verify_magic: failed to read flash\r\n");
		return false;
	}
	if (tmpmagic != magic)
	{
...
void FRAG_DECODER_IF_OnDone(int32_t status, uint32_t size, uint32_t *addr)
{
  ...
  uint32_t datafile = FOTA_DWL_REGION_START;

  if( !verify_magic (datafile, FIRMWARE_MAGIC) )
  {
